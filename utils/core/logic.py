...
def load_data_with_batching(data_source: str, batch_days: int = 7, **kwargs):
    """
    Load data by splitting date range into batches and re-aggregating metrics.
    
    This function splits the date range into batches, exports each batch separately,
    then re-aggregates metrics to avoid duplicates when merging.
    
    Args:
        data_source: Data source key
        batch_days: Number of days per batch (default: 7)
        **kwargs: Other SQL parameters
    
    Returns:
        Merged and re-aggregated DataFrame containing all batch data
    """
    # Get date parameters
    ...
    if not start_date or not end_date:
        # No date range, use regular load_data
        return load_data(data_source)
    
    # Split date range into batches
    from utils.core.batch_export import split_date_range_by_days, merge_and_reaggregate_by_config
    batches = split_date_range_by_days(start_date, end_date, batch_days)
    
    # Export progress tracking
    st.info(f"📊 Splitting export into {len(batches)} batch(es) of {batch_days} days each...")
    ...
    for i, (batch_start, batch_end) in enumerate(batches):
        ...
        batch_params = kwargs.copy()
        batch_params['start_date'] = batch_start
        batch_params['end_date'] = batch_end
        
        # Get data for this batch
        with st.spinner(f"Exporting batch {i+1}/{len(batches)} ({batch_start} to {batch_end})..."):
            batch_df = get_data("data", data_source, **batch_params)
            if batch_df is not None and not batch_df.empty:
                all_dataframes.append(batch_df)
    
    progress_bar.empty()
    
    # Merge and re-aggregate all batches to avoid metric duplicates
    if all_dataframes:
        merged_df = merge_and_reaggregate_by_config(all_dataframes, data_source)
        # Track final row count after re-aggregation for UI consistency
        try:
            st.session_state.final_row_count = int(len(merged_df))
        except Exception:
            st.session_state.final_row_count = None
        st.success(f"✅ Successfully merged {len(batches)} batches with proper metric aggregation!")
        return merged_df
    else:
        return pd.DataFrame()